...
function createConsortium() {
  ...
  set -x
  configtxgen -profile TesteOrgsOrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function networkUp() {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f ${COMPOSE_FILE_BASE}"
  fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    sudo rm -Rf /var/hyperledger/couchdbteste0/
  fi
}
...
# use typescript as the default language for chaincode
